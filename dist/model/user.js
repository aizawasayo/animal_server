...
const mongoose = require('mongoose'); // 导入bcrypt


const bcrypt = require('bcrypt');

const Joi = require('joi'); // 创建用户集合规则
...
const userSchema = new mongoose.Schema({
  ...
  email: {
    type: String // unique: true, // 保证邮箱地址不重复 还是靠api处理吧。。
    // required: true
...
const validateUser = user => {
  ...
  const schema = Joi.object({
    ...
    password: user._id ? Joi.string() : Joi.string().regex(/^[a-zA-Z0-9]{3,30}$/).required().error(new Error('密码格式不符合要求')),
